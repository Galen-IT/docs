...
6. Создайте директорию _"src/main/jniLibs/arm64-v8a"_ в корне вашего основного модуля и
   скопируйте туда предоставленный **libstage1.so**
7. Добавьте необходимые зависимости в файле **build.gradle** вашего основного модуля
   ```gradlew
      dependencies {
   ...
   ```
   ![Подключение библиотеки](../assets/config/step1/libs.jpg)
8. Синхронизируйте проект, чтобы применить изменения. Например, в Android Studio можно нажать Sync Now или выбрать в меню File → Synchronize. Дождитесь окончания синхронизации.
9. SDK готов к внедрению.
